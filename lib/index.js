...
const createAutocompleteIndexer = options => {
  ...
  const AutocompleteIndexer = {
    ...
    query(word, queryOptions) {
      queryOptions = Object.assign({count: 5}, queryOptions);
      const disassembled = Hangul.toString(word.toLowerCase());
      return new Promise((resolve, reject) => {
        ...
        return new Promise((resolve, reject) => {
          client.zrange(`${options.hashKey}:tokens:${disassembled.substr(0, 1)}`, index, index - 1 + (queryOptions.count * (options.maxCacheLength + 1)), (err, res) => err ? reject(err) : resolve(res));
        }).then(list => {
          if (!list || !list.length) {
            return Promise.resolve([]);
          }
          const keys = list.filter(key => key.endsWith('*') && key.startsWith(disassembled)).map(key => key.substr(0, key.length - 1)).filter((_, i) => i < 5);
          return new Promise((resolve, reject) => {
